...
const StoreDetails = () => {
    ...
    const fetchStore = async () => {
      try {
        ...
        if (storeData && storeData.id) {
          const reviewRes = await publicReviewService.getStoreReviews(
            storeData.id
          );
  ...
  const getMapUrl = (store) => {
    if (!store || !store.map_location_url)
      return "https://maps.google.com/maps?q=India&t=&z=13&ie=UTF8&iwloc=&output=embed";
              ...
              store.images.length > 0 && (
                <div className="store-gallery-section">
                  <h2 className="gallery-title">
                    Store Gallery ({store.images.length} images)
                  </h2>
            ...
            <div className="map-modern-wrapper">
              ...
              {store && (
                <div className="modern-map-info-overlay">
                  {store.logo && (
                    <div
                      className="modern-map-logo shimmer"
          ...
          >
            <form
              onSubmit={async (e) => {
                ...
                setSubmitSuccess("");
                try {
                  await publicReviewService.addStoreReview(
                    store?.id,
                    reviewForm
                  );
                  toast.success(
            ...
            >
              ...
              <input
                ...
                onChange={(e) =>
                  setReviewForm((f) => ({ ...f, phone_number: e.target.value }))
                }
                required
              />
              <select
      ...
      >
        ...
        >
          ...
          <textarea
            ...
            value={bookingForm.inquiry}
            onChange={(e) =>
              setBookingForm((f) => ({ ...f, inquiry: e.target.value }))
            }
            required
          />
          <button type="submit">Submit</button>
          {bookingSuccess && (
            <div style={{ color: "green" }}>{bookingSuccess}</div>
          )}
        </form>
      </Modal>
      <Modal
      ...
      >
        ...
        >
          <input
            ...
            value={helpForm.name}
            onChange={(e) =>
              setHelpForm((f) => ({ ...f, name: e.target.value }))
            }
            required
          />
          <input
            ...
            onChange={(e) =>
              setHelpForm((f) => ({ ...f, phone: e.target.value }))
            }
            required
          />
          <textarea
            placeholder="Your Query"
            value={helpForm.query}
            onChange={(e) =>
              setHelpForm((f) => ({ ...f, query: e.target.value }))
            }
            required
          />
          <button type="submit" disabled={submitting}>
